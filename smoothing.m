...
function [Xs,Ys] = smoothG(X,Y,s,n=0)    % Gaussian Smoothing
  ...
  if n==0   % El promediado se hace en todo el rango posible
    for i=1:N 
      ...
      Yc(i) = Gx*Y';                              % dependiendo de su distancia
    endfor
    Ys = Yc.*norm(Y)/norm(Yc);
    Xs = X;
  elseif n<0   % Los n negativos implica que se filtren puntos
    ...
    for i=1:(M-1)
      ...
      Yc(i) = Gx*Y(i*n:(i+1)*n)';            % punto dependiendo de su distancia
    endfor
    if n*M<N
      Xs(M) = sum(X(n*M:N))/n;
    ...
    for i=1:N
      ...
      Yc(i) = Gx*Y(max(1,i-floor(n/2)):min(N,i+ceil(n/2)))'/sum(Gx);   
